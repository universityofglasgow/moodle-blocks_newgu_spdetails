...
/**
 * Concrete implementation for mod_questionnaire.
 *
...
class questionnaire_activity extends base {
    ...
    public function __construct(int $gradeitemid, int $courseid, int $groupid) {
        ...
        $this->questionnaire = $this->get_questionnaire($this->cm);
    }
     ...
     * @return object
     */
    public function get_questionnaire($cm): object {
        global $DB;

        $coursemodulecontext = \context_module::instance($cm->id);
        $questionnaire = $DB->get_record('questionnaire', ['id' => $this->gradeitem->iteminstance], '*', MUST_EXIST);
        $questionnaire->coursemodulecontext = $coursemodulecontext;

        return $questionnaire;
    }

    /**
     * A questionnaire activity doesn't appear to end up in the Gradebook.
