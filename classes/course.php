...
class course {
    ...
    public static function get_cmid(string $cmodule, int $courseid, int $instance): int {
        ...
        $arrmodule = $DB->get_record('modules', array('name' => $cmodule));
        $moduleid = $arrmodule->id;

        $arrcoursemodule = $DB->get_record('course_modules', array(
            'course' => $courseid,
            'module' => $moduleid,
            'instance' => $instance,
        ));
                                ...
                                if ($cm->uservisible) {
                                    ...
                                    if ($assessments = $activityitem->get_assessmentsdue()) {
                                        ...
                                        if (($assessment->duedate != 0) && $assessment->duedate < $when) {
                                            ...
                                            $assessment_weight = self::return_weight($item->aggregationcoef);
